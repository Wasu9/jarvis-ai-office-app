...
function parseCountText(text: string): CountResult | null {
  ...
  const ranges = [/(?:questions?|प्रश्न)\s*(?:number|nos?\.?|from)?\s*(\d{1,4})\s*(?:to|through|–|—|-)\s*(\d{1,4})/i, /(?:\bQ(?:uestion)?\s*\.?\s*)?(\d{1,4})\s*(?:to|through|–|—|-)\s*(\d{1,4})\s*(?:questions?|प्रश्न)/i];
  for (const re of ranges) { const m = normalized.match(re); if (m && Number(m[1]) === 1) { const parsed = validCount(Number(m[2]), Number(m[2])); if (parsed) return parsed; } }
...
function detectCountFromQuestionSequence(text: string): CountResult | null {
  ...
  const patterns = [/\bQ(?:uestion)?\s*\.?\s*(\d{1,4})\s*[.)\-:]?/gi, /\bQuestion\s*(?:No\.?\s*)?(\d{1,4})\s*[.)\-:]?/gi, /(?:^|\s)(\d{1,4})\s*[.)\-:]\s+/gm];
...
function detectCountFromSectionRanges(text: string): CountResult | null {
  ...
  const patterns = [/(?:\(|\[)?\s*(\d{1,3})\s*(?:to|-)\s*(\d{1,3})\s*(?:\)|\])?\s*(?:[\u2013\u2014-]\s*)?(?:This section contains|Section contains)\s*(\d{1,3})\s*questions/gi, /(?:This section contains|Section contains)\s*(\d{1,3})\s*questions[\s\S]{0,120}?(?:\(|\[)?\s*(\d{1,3})\s*(?:to|-)\s*(\d{1,3})\s*(?:\)|\])?/gi];
  ...
  if (!ranges.length) return null; ranges.sort((a, b) => a.start - b.start); let expected = 1, last = 0; for (const range of ranges) { if (range.start !== expected) break; expected = range.end + 1; last = range.end; } return last > 0 ? { questionCount: last, lastQuestionNumber: last } : null;
}

async function enrichForCount(files: any[]): Promise<InlineFile[]> { return Promise.all(files.map(async (file: any) => { const base: InlineFile = { mimeType: file.type || 'application/pdf', data: stripDataUrl(file.base64Data || ''), name: file.name, textPreview: typeof file.textPreview === 'string' ? file.textPreview : undefined }; if (base.textPreview?.trim() || !file.base64Data) return base; try { const markdown = await convertFileToMarkdown(file); if (markdown) return { ...base, textPreview: markdown.slice(0, 120_000) }; } catch (error) { console.warn(`[MarkItDown] Source-count conversion failed for ${file.name}; continuing with Gemini PDF input.`, error); } return base; })); }
...
async function askCountFromText(provider: any, extractedText: string, model: string): Promise<CountResult | null> {
  ...
  try { const raw = await provider.generateText(`Determine the ACTUAL number of numbered questions in the source text below. Ignore MCQ option numbers, page numbers, section numbers and marks. Find the real sequence beginning at Q1 and identify the final question number. Never assume 180. Return ONLY JSON: {"questionCount": number, "lastQuestionNumber": number}.\n\nSOURCE TEXT:\n${textForAI}`, { model, responseMimeType: 'application/json', responseSchema: COUNT_SCHEMA, inlineFiles: [], skipDocumentContext: true }); try { const data = extractJson(raw); const parsed = validCount(Number(data?.questionCount), Number(data?.lastQuestionNumber)); if (parsed) return parsed; } catch {} return parseCountText(raw); } catch (error) { console.warn('[Source Count] text-only Gemini count failed:', error); return null; }
}
...
async function askCountFromVision(provider: any, inline: InlineFile[], model: string): Promise<CountResult | null> {
  ...
  try { const raw = await provider.generateText('Inspect the attached original PDF visually. Determine the ACTUAL final printed question number and total number of real numbered questions. Ignore option labels, page numbers, section numbers and marks. Verify the sequence starts at Q1. Never assume 180. Return ONLY JSON: {"questionCount": number, "lastQuestionNumber": number}.', { model, responseMimeType: 'application/json', responseSchema: COUNT_SCHEMA, inlineFiles: visualFiles.map(f => ({ mimeType: f.mimeType, data: f.data })), skipDocumentContext: true }); try { const data = extractJson(raw); const parsed = validCount(Number(data?.questionCount), Number(data?.lastQuestionNumber)); if (parsed) return parsed; } catch {} return parseCountText(raw); } catch (error) { console.error('[Source Count] visual Gemini count failed:', error); return null; }
}
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const extractedText = inline.map(f => f.textPreview?.trim() || '').filter(Boolean).join('\n\n');
    ...
    const provider = aiRegistry.getProvider('gemini'), model = req.body?.model || 'gemini-3.8-flash';
  ...
  } catch (err: any) { console.error('[Source Count Error]', err); return res.status(500).json({ error: err?.message || 'Failed to detect source question count.' }); }
}
