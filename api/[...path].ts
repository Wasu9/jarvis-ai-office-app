...
app.get('/api/providers', (_req, res) => res.json({
  providers: aiRegistry.listProviders(), current: 'gemini', isApiKeySet: !!process.env.GEMINI_API_KEY,
}));
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const safeHistory = history.filter((m: any) => (m?.role === 'user' || m?.role === 'assistant') && typeof m?.content === 'string').map((m: any) => `${m.role === 'user' ? 'User' : 'JARVIS'}: ${m.content}`).join('\n');
    const memory = memoryStore.getAll().slice(-12).map((m: any) => `${m.key}: ${m.value}`).join('\n');
    const systemInstruction = `You are JARVIS, a highly capable personal AI assistant for Shaheen Academy Jaipur. Behave like a natural, intelligent human assistant: warm, concise, context-aware, proactive and practical. Match the user's Hindi, English or Hinglish naturally. Do not speak like a paper generator, API, robot, programmer or system log. For casual conversation, answer casually. For questions, explain clearly and directly. If the user asks what you can do, describe capabilities naturally. If a request is ambiguous, ask only the minimum useful clarification. Do not invent completed actions, files, browsing, memory or capabilities you did not actually perform. You are the conversational layer; complex production tasks are handled separately by specialist employees. Never output JSON unless the user explicitly asks for JSON. Keep ordinary replies concise (normally 1-5 short paragraphs). Current date: ${new Date().toISOString().slice(0,10)}.\nRelevant saved memory:\n${memory || 'None'}\nRecent conversation:\n${safeHistory || 'None'}`;
    const reply = await aiRegistry.getProvider('gemini').generateText(message, { model: req.body?.model || 'gemini-3.7-flash', systemInstruction });
    res.json({ reply, mode: 'conversation' });
  } catch (err: any) {
    console.error('[Chat Error]', err);
    res.status(500).json({ error: err?.message || 'JARVIS could not respond right now.' });
  }
});

app.post('/api/tasks/route-check', (req, res) => {
  try { const { prompt, attachedFiles, selectedAgentId } = req.body; const routedAgentId = TaskRunner.routeAgent(prompt || '', attachedFiles, selectedAgentId); res.json({ routedAgentId, agent: agentRegistry.getAgent(routedAgentId) }); }
  catch (err: any) { res.status(500).json({ error: err.message }); }
});
