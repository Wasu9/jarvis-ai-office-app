...
a = s.index('function richChildren')
...
rich = '''function richChildren(text: string, options: TextOptions = {}): Array<TextRun | DocxMath> {
  ...
  while ((match = pattern.exec(source))) {
    ...
    if (before) output.push(baseRun(before));
...
qstart = s.index('async function questionTable')
qend = s.index('\nfunction appendAnswerKey', qstart)
q = s[qstart:qend]
if 'if (needsSourceImage(q))' not in q:
    needle = "  const hi: any[] = [textParagraph(cleanQ(q.textHi || ''), { bold: true, size: QUESTION_SIZE, color: '334155', hindi: true, after: 45, keepNext: true })];"
    block = "\n  if (needsSourceImage(q)) { const ph = await renderDiagram(sourceImagePlaceholderSvg(q.number)); if (ph) { en.push(ph); const hp = await renderDiagram(sourceImagePlaceholderSvg(q.number)); if (hp) hi.push(hp); } } else if (q.diagramSvg?.trim()) { const diagram = await renderDiagram(q.diagramSvg); if (diagram) { en.push(diagram); const hindiDiagram = await renderDiagram(q.diagramSvg); if (hindiDiagram) hi.push(hindiDiagram); } }"
    if needle in q:
        q = q.replace(needle, needle + block, 1)
    else:
        raise SystemExit('questionTable insertion point not found')
s = s[:qstart] + q + s[qend:]
p.write_text(s)
