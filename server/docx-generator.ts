...
/** Render LaTeX/native fraction and common PDF-style slash fractions as editable Word math. */
function richChildren(text:string,options:TextOptions={}):Array<TextRun|DocxMath>{
  const source=String(text||'');
  const children:Array<TextRun|DocxMath>=[];
  const fraction=/\\frac\{([^{}]+)\}\{([^{}]+)\}|\(([^()]+)\)\s*\/\s*\(([^()]+)\)|\b([A-Za-zα-ωΑ-Ω0-9]+)\s*\/\s*([A-Za-zα-ωΑ-Ω0-9]+)\b/g;
  let last=0; let match:RegExpExecArray|null;
  while((match=fraction.exec(source))){
    if(match.index>last) children.push(...richTextSegment(source.slice(last,match.index),options));
    const numerator=match[1]??match[3]??match[5]??''; const denominator=match[2]??match[4]??match[6]??'';
    children.push(new DocxMath({children:[new MathFraction({numerator:[mathText(numerator)],denominator:[mathText(denominator)]})]}));
    last=match.index+match[0].length;
  }
  if(last<source.length) children.push(...richTextSegment(source.slice(last),options));
  return children.length?children:[new TextRun({text:'',size:options.size})];
}

function richTextSegment(source:string,options:TextOptions):Array<TextRun|DocxMath>{
  const out:Array<TextRun|DocxMath>=[]; let last=0;
  const power=/([A-Za-zα-ωΑ-Ω0-9])\^\{([^{}]+)\}|([A-Za-zα-ωΑ-Ω0-9])\^([0-9+\-=]+)/g; let m:RegExpExecArray|null;
  while((m=power.exec(source))){
    if(m.index>last) out.push(new TextRun({text:cleanText(source.slice(last,m.index)),bold:options.bold,size:options.size,font:options.hindi?'Noto Sans Devanagari':'Aptos',color:options.color}));
    const base=m[1]??m[3]??''; const exp=m[2]??m[4]??'';
    out.push(new DocxMath({children:[mathSuper(base,exp)]}));
    last=m.index+m[0].length;
  }
  if(last<source.length) out.push(new TextRun({text:cleanText(source.slice(last)),bold:options.bold,size:options.size,font:options.hindi?'Noto Sans Devanagari':'Aptos',color:options.color}));
  return out;
}
...
function cell(children:Paragraph[],width:number,fill?:string){return new TableCell({width:{size:width,type:WidthType.PERCENTAGE},shading:fill?{fill,type:ShadingType.CLEAR}:undefined,margins:{top:45,bottom:45,left:100,right:100},children});}
...
async function svgToPng(svg:string):Promise<{data:Buffer;width:number;height:number}> {
  ...
  const originalW=Math.max(1,Number(meta.width)||560), originalH=Math.max(1,Number(meta.height)||280);
  const width=Math.max(160,Math.min(560,originalW));
...
async function diagramParagraph(svg?:string):Promise<Paragraph|null>{
  ...
  try{const rendered=await svgToPng(svg);return new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:40,after:45},children:[new ImageRun({type:'png',data:rendered.data,transformation:{width:rendered.width,height:rendered.height}})]});}catch{return null;}
}

async function questionTable(q:NonNullable<DocxPaperData['questions']>[number],addHeader:boolean):Promise<Table>{
  const english:Paragraph[]=[textParagraph(`Q.${q.number}  ${q.textEn}`,{bold:true,size:24,color:'0F172A',after:45,keepNext:true})];
  const hindi:Paragraph[]=[textParagraph(q.textHi||'',{bold:true,size:20,color:'334155',hindi:true,after:45,keepNext:true})];
  const diagram=await diagramParagraph(q.diagramSvg);
  if(diagram){
    const diagram2=await diagramParagraph(q.diagramSvg);
    if(diagram2) english.push(diagram2);
    if(diagram) hindi.push(diagram);
  }
  const en=q.optionsEn||[],hi=q.optionsHi||[];
  for(let i=0;i<4;i++){
    english.push(textParagraph(`(${i+1}) ${en[i]||''}`,{size:24,after:14}));
    hindi.push(textParagraph(`(${i+1}) ${hi[i]||''}`,{size:20,hindi:true,after:14}));
  }
  const rows:TableRow[]=[];
...
export async function generateDocxBuffer(data:DocxPaperData):Promise<Buffer>{
  ...
  children.push(textParagraph(instituteName,{bold:true,size:28,color:'164E63',align:AlignmentType.CENTER,after:15}));
  children.push(textParagraph(title,{bold:true,size:24,color:'0F172A',align:AlignmentType.CENTER,after:30}));
  if(questions.length){
    ...
    if(metaParts.length) children.push(textParagraph(metaParts.join('   ·   '),{size:18,color:'475569',align:AlignmentType.CENTER,after:45}));
    if(data.instructions?.length){children.push(textParagraph('GENERAL INSTRUCTIONS / सामान्य निर्देश:',{bold:true,size:20,color:'334155',after:20,keepNext:true}));for(const instruction of data.instructions)children.push(textParagraph(instruction,{size:20,color:'475569',after:14}));}
    for(let index=0;index<questions.length;index++){
      const q=questions[index];
      children.push(await questionTable(q,index===0));
    }
    if(data.includeAnswerKey){const keyed=questions.filter(q=>q.correctOption);if(keyed.length){children.push(textParagraph('ANSWER KEY / उत्तर कुंजी',{bold:true,size:22,color:'164E63',align:AlignmentType.CENTER,before:80,after:30}));const width=100/Math.min(10,keyed.length);children.push(new Table({width:{size:100,type:WidthType.PERCENTAGE},rows:[new TableRow({cantSplit:true,children:keyed.map(q=>cell([textParagraph(`Q.${q.number}`,{bold:true,size:16,align:AlignmentType.CENTER,after:0})],width,'E2E8F0'))}),new TableRow({cantSplit:true,children:keyed.map(q=>cell([textParagraph(String(q.correctOption||'-').replace(/^A$/i,'1').replace(/^B$/i,'2').replace(/^C$/i,'3').replace(/^D$/i,'4'),{bold:true,size:20,color:'15803D',align:AlignmentType.CENTER,after:0})],width))})]}));}}
    if(data.includeSolutions){const solved=questions.filter(q=>q.solution);if(solved.length){children.push(textParagraph('DETAILED SOLUTIONS / विस्तृत हल',{bold:true,size:22,color:'164E63',align:AlignmentType.CENTER,before:80,after:30}));for(const q of solved){children.push(textParagraph(`Q.${q.number} Solution / हल:`,{bold:true,size:20,color:'2563EB',after:15,keepNext:true}));children.push(textParagraph(q.solution||'',{size:20,after:22}));}}}
  ...
  const doc=new Document({creator:'JARVIS AI Office',title,sections:[{properties:{page:{margin:{top:520,bottom:520,left:650,right:650}}},children}]});
