...
function latexComponents(latex:string):MathComponent[] {
  ...
  const flush=()=>{if(buffer){out.push(new MathRun({text:cleanPlainText(buffer),font:FONT_MATH,size:BODY_SIZE}));buffer='';}};
  const attachScript=(kind:'sup'|'sub',value:string)=>{const base=out.pop()||new MathRun({text:'',font:FONT_MATH,size:BODY_SIZE});const script=latexComponents(value);if(kind==='sup')out.push(new MathSuperScript({children:[base],superScript:script}));else out.push(new MathSubScript({children:[base],subScript:script}));};
  for(let i=0;i<source.length;){
    ...
    if(source[i]==='\\'){const match=source.slice(i+1).match(/^([A-Za-z]+)/);if(match){const command=match[1];if(GREEK[command]){flush();out.push(new MathRun({text:GREEK[command],font:FONT_MATH,size:BODY_SIZE}));i+=command.length+1;continue;}const symbols:Record<string,string>={times:'×',cdot:'·',pm:'±',mp:'∓',leq:'≤',geq:'≥',neq:'≠',propto:'∝',infty:'∞',approx:'≈',rightarrow:'→',to:'→',degree:'°',sin:'sin',cos:'cos',tan:'tan',log:'log',ln:'ln',left:'',right:''};if(command in symbols){flush();if(symbols[command])out.push(new MathRun({text:symbols[command],font:FONT_MATH,size:BODY_SIZE}));i+=command.length+1;continue;}}}
    buffer+=source[i++];
  }
  flush();return out.length?out:[new MathRun({text:'',font:FONT_MATH,size:BODY_SIZE})];
}
...
function richChildren(text:string,options:TextOptions={}):Array<TextRun|DocxMath>{
  const source=sanitizeTextInput(String(text||'')); const baseRun=(value:string)=>new TextRun({text:cleanPlainText(value),bold:options.bold,size:options.size??BODY_SIZE,font:options.hindi?FONT_HI:FONT_EN,color:options.color});
  if(!hasMath(source))return[baseRun(source)]; const output:Array<TextRun|DocxMath>=[];const pattern=/\[\[MATH:([\s\S]*?)\]\]|\$\$([\s\S]*?)\$\$|\$([^$\n]+)\$|\\\(([\s\S]*?)\\\)/g;let last=0;let match:RegExpExecArray|null;
  while((match=pattern.exec(source))){if(match.index>last)output.push(baseRun(source.slice(last,match.index)));output.push(mathNode(match[1]??match[2]??match[3]??match[4]??''));last=match.index+match[0].length;}if(last<source.length)output.push(baseRun(source.slice(last)));return output.length?output:[baseRun('')];
}
...
function stripLeadingLabel(text:string,label?:number):string{let s=sanitizeTextInput(text).trim();s=s.replace(/^\s*[\(\[]\s*[1-4]\s*[\)\]]\s*/i,'').replace(/^\s*(?:Option|विकल्प)\s*[1-4]\s*[:.\-]\s*/i,'');if(label!==undefined)s=s.replace(new RegExp(`^\\s*${label}\\s*[.)]\\s*`),'');return s.trim();}
...
async function questionTable(q:NonNullable<DocxPaperData['questions']>[number],first:boolean):Promise<Table>{const cleanQ=(s:string)=>sanitizeTextInput(s).trim().replace(/^\s*Q\.?\s*\d+\s*[:.)-]?\s*/i,'');const en:any[]=[textParagraph(`Q.${q.number}  ${cleanQ(q.textEn)}`,{bold:true,size:QUESTION_SIZE,color:'0F172A',after:45,keepNext:true,pageBreakBefore:first})];const hi:any[]=[textParagraph(cleanQ(q.textHi||''),{bold:true,size:QUESTION_SIZE,color:'334155',hindi:true,after:45,keepNext:true})];const diagram=await renderDiagram(q.diagramSvg);if(diagram){en.push(diagram);const hindiDiagram=await renderDiagram(q.diagramSvg);if(hindiDiagram)hi.push(hindiDiagram);}if(q.questionType!=='numerical'){const enOptions=q.optionsEn||[];const hiOptions=q.optionsHi||[];const mtEn=isMatchColumn(q.textEn,enOptions)?matchTable(enOptions,false):null;const mtHi=isMatchColumn(q.textHi||'',hiOptions)?matchTable(hiOptions,true):null;if(mtEn&&mtHi){en.push(mtEn);hi.push(mtHi);}else{en.push(optionTable(enOptions,false));hi.push(optionTable(hiOptions,true));}}const rows:TableRow[]=[];if(first)rows.push(new TableRow({cantSplit:true,children:[makeCell([textParagraph('ENGLISH',{bold:true,size:BODY_SIZE,color:'FFFFFF',align:AlignmentType.CENTER,after:0})],50,'164E63'),makeCell([textParagraph('हिन्दी',{bold:true,size:BODY_SIZE,color:'FFFFFF',hindi:true,align:AlignmentType.CENTER,after:0})],50,'164E63')]}));rows.push(new TableRow({cantSplit:true,children:[makeCell(en,50,'F8FAFC'),makeCell(hi,50,'F8FAFC')]}));return new Table({width:{size:100,type:WidthType.PERCENTAGE},rows,borders:{...THIN_BORDERS,insideHorizontal:{style:BorderStyle.SINGLE,size:2,color:'E2E8F0'},insideVertical:{style:BorderStyle.SINGLE,size:4,color:'CBD5E1'}}});}
