...
export interface ExecuteTaskParams { userPrompt:string; selectedAgentId?:string; attachedFiles?:AttachedFile[]; model?:string; settings?:{instituteName?:string;targetExam?:string;defaultTargetExam?:string;primaryLanguage?:string;aiModel?:string}; onStep?:(step:TaskStep)=>void; resumeFrom?:number; resumeQuestions?:any[]; }
...
export class TaskRunner{
 ...
 static async execute(params:ExecuteTaskParams):Promise<TaskRecord>{
  ...
  let agent:any;
  let resumeCheckpoint:any=undefined;
  const fail=(message:string)=>{add('failed',message);const task=this.failed(id,start,params,agent,steps,message,resumeCheckpoint);try{validateTaskRecord(task);saveTask(task);}catch{}return task;};
  ...
  add('generating',locked?(params.resumeFrom&&params.resumeFrom>1?`Resuming source · Q.${params.resumeFrom}+`:'Processing source · parallel extraction mode'):`Generating with ${params.model||'Gemini 3.7 Flash'}`);
  ...
  try{
   if(structured&&locked&&inline.length){
    const requested=countRequested(params.userPrompt);const totalQ=requested&&requested>0?requested:180;const chunkSize=20;const resumeFrom=Math.min(totalQ,Math.max(1,Number(params.resumeFrom||1)));const initialQuestions=Array.isArray(params.resumeQuestions)?params.resumeQuestions:[];const chunks:Array<{start:number;end:number}>=[];
    for(let startQ=resumeFrom;startQ<=totalQ;startQ+=chunkSize)chunks.push({start:startQ,end:Math.min(startQ+chunkSize-1,totalQ)});
    const merged:any={title:'',instituteName:'',examType:'',subject:'',duration:'',totalMarks:'',instructions:[],questions:[...initialQuestions]};
    resumeCheckpoint={completedQuestions:Math.max(0,resumeFrom-1),totalQuestions:totalQ,nextQuestion:resumeFrom,questions:[...merged.questions]};
    const extractChunk=async(c:{start:number;end:number})=>{
      let lastError='';
      for(let attempt=1;attempt<=3;attempt++){
       try{
        add('working',`SOURCE EXTRACTION · Q.${c.start}–Q.${c.end}`,`Batch ${Math.floor((c.start-1)/chunkSize)+1} · attempt ${attempt}`);
    ...
    for(let i=0;i<chunks.length;i+=concurrency){
      ...
      const completed=Math.min(merged.questions.length,totalQ);resumeCheckpoint={completedQuestions:completed,totalQuestions:totalQ,nextQuestion:completed+1,questions:[...merged.questions]};
      add('checking',`SOURCE CHECKPOINT · ${completed}/${totalQ} questions captured`,`Checkpoint saved in the live task. Resume starts from Q.${completed+1}.`);
    }
